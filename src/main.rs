...
fn default_config_path() -> PathBuf {
    PathBuf::from("/opt/drive-backup/config.yml")
}
...
fn cmd_remote_auth(cfg: &Config, logger: &Logger) -> Result<(), String> {
...
fn cmd_restore(cfg: &Config, logger: &Logger, args: &[String]) -> Result<(), String> {
    ...
    if args.is_empty() {
        ...
        println!("\nUsage: backup-mgr restore <filename> [target-dir]");
        return Ok(());
    }

    let file = &args[0];
    let default_target = cfg.resolve(&cfg.inner.backup.dir).join("restore");
    let target_str = args.get(1).cloned().unwrap_or_else(|| default_target.to_string_lossy().to_string());
    let target = cfg.resolve(&target_str);

    if let Err(e) = cmd_restore_do(cfg, logger, file, &target) {
        ...
        notify::send(
            ...
            logger,
            &notif_ctx(cfg),
            &notify::Event::RestoreFailure { name: file.clone(), reason: e.clone() },
        );
        if let Some(d) = db() {
    ...
    notify::send(
        ...
        logger,
        &notif_ctx(cfg),
        &notify::Event::RestoreSuccess { name: file.clone(), size, target: target.display().to_string() },
    );
    if let Some(d) = db() {
...
fn cmd_restore_do(cfg: &Config, logger: &Logger, file: &str, target: &Path) -> Result<(), String> {
    ...
    // verify against manifest before extracting
    if let Some(d) = db() {
        if let Some(m) = d.manifest(file) {
            match drive::remote_md5(&r, file) {
                Ok(remote_hash) if remote_hash == m.md5 => {
                    logger.info(&format!("integrity pre-check passed for {} (md5 matches manifest)", file));
                }
                Ok(remote_hash) => {
                    let warn = format!("integrity pre-check FAILED for {}: remote md5 {} != manifest md5 {}", file, remote_hash, m.md5);
                    logger.warn(&warn);
                }
                Err(_) => {}
            }
...
fn main() {
    ...
    let result: Result<(), String> = match cmd.as_str() {
        ...
        "restore" => cmd_restore(&cfg, &logger, &rest),
        "check" => {
